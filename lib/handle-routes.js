...
module.exports = async function (options) {
  const { _, getNdutConfig, aneka } = this.ndut.helper
  const { build } = this.ndutRoute.helper

  let scanDirs = []
  scanDirs = _.concat(scanDirs, options.scan || [])
  await build(this, { name: 'ndutRest', scanDirs, prefix: options.prefix, notFoundMsg: 'Resource not found', customBuilder })
